...
class SensorEmbedding(ABC):
    ...
    def __init__(
        ...
        self.sensor_positions: torch.Tensor = sensor_positions.float().to(self.device)
...
class Voronoi(SensorEmbedding):
    ...
    def __call__(self, data: torch.Tensor, seed: int = 0) -> torch.Tensor:
        ...
        n_dropout_sensors: torch.LongTensor = torch.multinomial(
            torch.tensor(self.dropout_probabilities, device=self.device),
            num_samples=N * T, replacement=True,
        ).reshape(N, T)
        # Precompute dropout masks across all samples and frames (fast)
        masks: torch.Tensor = torch.ones((N, T, self.S), dtype=torch.bool, device=self.device)
        for i in range(N):
            for t in range(T):
                n_sensors_to_drop: int = n_dropout_sensors[i, t].item()
                if n_sensors_to_drop > 0:
                    dropout_indices = torch.randperm(self.S, device=self.device)[:n_sensors_to_drop]
    ...
    def __call__(self, data: torch.Tensor, seed: int = 0) -> torch.Tensor:
        ...
        for i in range(N):
            for t in range(T):
                ...
                dropout_indices: torch.Tensor = torch.randperm(self.S, device=self.device)[:n_dropout_sensors]
...
class Vector(SensorEmbedding):
    ...
    def __call__(self, data: torch.Tensor, seed: int = 0) -> torch.Tensor:
        ...
        torch.manual_seed(seed)
        noisy_data: torch.Tensor = data + torch.randn_like(data) * self.noise_level * data.abs()
        output: torch.Tensor = torch.zeros((N, T, C, self.S), dtype=torch.float, device=self.device)
        for i in range(N):
            for t in range(T):
                n_dropout_sensors: int = random.choices(
                    population=range(0, self.n_max_dropout_sensors + 1, 1), weights=self.dropout_probabilities, k=1
