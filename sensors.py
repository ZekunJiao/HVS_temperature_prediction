...
class LHS(SensorGenerator):
    ...
    def __call__(self) -> torch.Tensor:
        ...
        # absolute positions
        sensor_positions = torch.from_numpy(lhs_samples).to(self.device) * torch.tensor(data=self.resolution, device=self.device)
...
class AroundCylinder(SensorGenerator):
    ...
    def __call__(
        ...
        np.random.seed(self.seed)
        lhs = LHS(n_sensors=self.n_sensors, device=self.device); lhs.resolution = (360,)
        ...
        sensor_positions: torch.Tensor = torch.zeros((self.n_sensors, len(self.resolution)), dtype=torch.int32, device=self.device)
        sensor_positions[:, 0] = torch.from_numpy(np.cos(np.deg2rad(samples))).to(self.device) * radius_h_pixels + center_h_pixels
        sensor_positions[:, 1] = torch.from_numpy(np.sin(np.deg2rad(samples))).to(self.device) * radius_w_pixels + center_w_pixels
...
if __name__ == '__main__':
    ...
    resolution = (140, 240)
    lhs = LHS(n_sensors=32, device=device)
    lhs.resolution = resolution
    ac = AroundCylinder(n_sensors=32, device=device)
    ...
    for name, sensor_positions in zip(('lhs', 'ac'), (a, b)):
        ...
        x_coords = sensor_positions[:, 1].cpu().numpy()
        y_coords = sensor_positions[:, 0].cpu().numpy()
